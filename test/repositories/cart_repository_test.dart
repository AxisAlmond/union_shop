...
void main() {
  ...
  test('addItem merges duplicates and floors quantity at 1', () {
    repo.addItem(product: _product(), quantity: 0, selectedSize: 'M', selectedColor: 'Red');
    repo.addItem(product: _product(), quantity: 2, selectedSize: 'M', selectedColor: 'Red');
  ...
  test('addItem keeps separate entries for different options', () {
    repo.addItem(product: _product(), quantity: 1, selectedSize: 'M', selectedColor: 'Red');
    repo.addItem(product: _product(), quantity: 1, selectedSize: 'L', selectedColor: 'Red');
    repo.addItem(product: _product(), quantity: 1, selectedSize: 'M', selectedColor: 'Blue');

    expect(repo.items.length, 3);
  });

  test('updateQuantity removes when newQuantity <= 0', () {
    repo.addItem(product: _product(), quantity: 1);
  ...
  test('totals compute correctly', () {
    repo.addItem(product: _product(price: 5.0), quantity: 2);
    repo.addItem(product: _product(title: 'P2', price: 7.5), quantity: 1);
